...
壊されるのではなく更新がブロックされます）。エディターのログイン情報も自動で再設定されます。
...
手軽にデザインを変えるなら、エディターの **Settings → Theme** を選ぶのが一番簡単です。
